...
- rubocop-railsの導入
- 汎用モーダルの作成
- 学生が提出したものに対する教員からのフィードバックをExcelファイルを用いて一括で行う機能追加
  - Excel読み込み(gem rooを使用)
  - バリデーション
  - 成功時は画面遷移してフォームにExcelファイル内のフィードバックが入力された状態
  - 失敗時は失敗行を返してテーブル表示
  - 成功時に遷移した画面で一括送信ボタンを押してフォームに入力されている学生宛にメッセージ送信
  - 成功時に遷移した画面で個別に送信することも可能
